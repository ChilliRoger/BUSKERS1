...
/**
 * @title MusicNFTV2
 * @dev ERC-721 token for music files with PYUSDC payment integration
 * @notice This contract allows public minting with PYUSDC payment
 */
contract MusicNFTV2 is ERC721, ERC721URIStorage, Ownable {
    uint256 private _nextTokenId;
    
    // Base URI for token metadata (can be overridden)
    string private _baseTokenURI;
    
    // PYUSD token contract (PayPal USD)
    IERC20 public immutable pyusd;
    
    // Minting price in PYUSD (6 decimals like USDC)
    uint256 public mintPrice;
    
    // Token prices for selling (tokenId => price in PYUSD)
    mapping(uint256 => uint256) public tokenPrices;
    
    // Events
    event MusicMinted(address indexed to, uint256 indexed tokenId, string tokenURI, uint256 price);
    ...
    /**
     * @dev Mint a new music NFT with PYUSD payment
     * @param to The address to mint the NFT to
     * @param tokenUriString The pre-formatted token URI
    ...
    function mint(address to, string memory tokenUriString) public returns (uint256) {
        ...
        require(pyusd.allowance(msg.sender, address(this)) >= mintPrice, "MusicNFT: insufficient PYUSD allowance");
        ...
        emit MusicMinted(to, tokenId, tokenUriString, mintPrice);
     ...
     * @param tokenId The token ID
     * @param price The price in PYUSDC
     */
    ...
    function buyNFT(uint256 tokenId, address buyer, uint256 amount) public {
        ...
        // Transfer PYUSD from buyer to current owner
        require(
            pyusd.transferFrom(buyer, currentOwner, tokenPrices[tokenId]),
            "MusicNFT: PYUSD transfer failed"
        );
    ...
    /**
     * @dev Update minting price (only owner)
     * @param newPrice The new minting price in PYUSD
     */
    ...
    /**
     * @dev Withdraw PYUSD from contract (only owner)
     * @param amount The amount to withdraw
     */
    function withdrawPYUSD(uint256 amount) public onlyOwner {
        require(pyusd.balanceOf(address(this)) >= amount, "MusicNFT: insufficient contract balance");
        require(pyusd.transfer(owner(), amount), "MusicNFT: PYUSD transfer failed");
    }
    
    /**
     * @dev Get contract's PYUSD balance
     * @return The contract's PYUSD balance
     */
