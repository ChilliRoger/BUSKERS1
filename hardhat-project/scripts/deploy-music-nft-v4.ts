...
async function main() {
  console.log("🚀 Deploying MusicNFTV4 contract with PYUSD payment...");

  // Get the contract factory
  const MusicNFTV4 = await ethers.getContractFactory("MusicNFTV4");
  ...
  const mintPrice = ethers.parseUnits("10", 6); // 10 PYUSD (6 decimals)

  // Deploy the contract
  const musicNFT = await MusicNFTV4.deploy(
    name,
    symbol,
    baseURI,
    pyusdAddress,
  ...
  console.log("✅ MusicNFTV4 deployed successfully!");
  console.log("Contract Address:", contractAddress);
  console.log("Network:", (await ethers.provider.getNetwork()).name);
  console.log("Chain ID:", (await ethers.provider.getNetwork()).chainId);
  console.log("PYUSD Address:", pyusdAddress);
  console.log("Mint Price:", ethers.formatUnits(mintPrice, 6), "PYUSD");
  ...
  try {
    ...
    const pyusdContract = await musicNFT.pyusd();
    ...
    console.log("  Mint Price:", ethers.formatUnits(mintPriceFromContract, 6), "PYUSD");
  } catch (error) {
  ...
  console.log("3. Test PYUSD minting functionality");
  console.log("4. Verify on Kadena Explorer");
  console.log("5. Test faucet to get PYUSD tokens");
}
